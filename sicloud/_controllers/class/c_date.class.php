...
class c_date
{
    public static function m_add_time(string $timestamp, $timeToAdd, $formato = 'Y-m-d', $espanol = false, $operacion = '+')
    {
        ...
        if ($espanol) {
            foreach ($timeToAdd as $key => $value) {
                $timeToAdd[$key] = self::m_traduce_time($value, false);
            }
        }
    ...
    public static function m_dias_no_habiles(string $fechaI, string $fechaF, $dias = ['sábados','domingos','festivos'], $a = false)
    {
        ...
        while ($fechaInicial <= $fechaFinal) {
            ...
            if (in_array('sábados', $dias) && $a && $fechaInicial->format('l') === 'Saturday') {
        // Crea array con sabados
                  $aSabados[] = $fechaInicial->format('Y-m-d');
            }

            ( $fechaInicial->format('l') === 'Sunday' )   ? $domingos++ : $domingos += 0;
            if (in_array('domingos', $dias) && $a && $fechaInicial->format('l') === 'Sunday') {
    // Crea array con domingos
                $aDomingos[] = $fechaInicial->format('Y-m-d');
            }
          //Si está en el array y es sábado o domingo (Se restan esos días)
            if (array_key_exists($fechaInicial->format('Y-m-d'), $festivos)) {
            // Crea array con festivos
...
//Incremento de la fecha inicial
        }
        ...
        if ($a) {
            ...
            if (count($aDomingos) > 0) {
                $r['domingos'] = $aDomingos;
            }
            if (count($aFestivos) > 0) {
                $r['festivos'] = $aFestivos;
            }
        }
        return $r;
    }
...
// set the interval as 1 day
        if (empty($aFestivos)) {
            $aFestivos = self::obj('diasFeriados');
        }
    ...
    public static function m_dias_en_rango_time(string $fechaI, string $fechaF, string $returDataEn = 'días', string $recorreEn = 'días', $formato = null, $boolGroup = false)
    {
        ...
        for ($i = $fechaI; $i <= $fechaF; $i += $seg) {
            switch ($returDataEn) {
                ...
                case 'seconds':
                    if ($boolGroup) {
                        $aFechas[date('s', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('s', $i)]    = date($formato, $i);
                    }
                ...
                case 'minute':
                    if ($boolGroup) {
                        $aFechas[date('i', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('i', $i)]    = date($formato, $i);
                    }
                ...
                case 'hours':
                    if ($boolGroup) {
                        $aFechas[date('H', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('H', $i)]    = date($formato, $i);
                    }
                ...
                case 'dia':
                    if ($boolGroup) {
                        $aFechas[date('Y-m-d', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('Y-m-d', $i)]    = date($formato, $i);
                    }
                ...
                case 'weeks':
                    if ($boolGroup) {
                        $aFechas[date('W', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('W', $i)]    = date($formato, $i);
                    }
                ...
                case 'month':
                    if ($boolGroup) {
                        $aFechas[date('m', $i)][]  = date($formato, $i);
                    }
                    if ($boolGroup == false) {
                        $aFechas[date('m', $i)]    = date($formato, $i);
                    }
   ...
   //Transforma a una fecha legible un timestamp de forma "2019-06-16 22:52:00"   else   c_date::m_format_time($timestamp, 'H:m');
    ...
    public static function m_time_diff(string $timesInicial, string $timesFinal, string $formato = 'a')
    {
        if (strtotime($timesInicial) > strtotime($timesFinal)) {
            if ($formato == 'a') {
                ['status' => 'error', 'msg' => 'error fecha fina menor que la inicial'];
            }
        } else {
            $r = 0;
        }
