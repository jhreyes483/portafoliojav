...
class c_Festivos{
   ...
   function f_esFestivo($fecha, $fiesta=0){
      if(is_numeric($fecha)) $fecha = date('Y-m-d', $fecha);
      if(!$this->esFecha($fecha)) return false;
      $val = 0;
      $dia = date('N', strtotime($fecha));
      if($dia==7) $val = 1; //Es Festivo
      if($dia==6) $val = 2; //Es Sábado

      if(array_key_exists($fecha, $this->arrFes))  $val = 1;
   ...
   function esFecha($fecha) {
		return (boolean)strtotime($fecha);
	}
}
?>
