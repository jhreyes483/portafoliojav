...
class C_sanear{
   ...
   protected function verificaResult($a, $tipo=1){
      switch ($tipo) {
         case 0:
            if(count($a) != 0 ) return ['response_status'=>'ok', 'response_msg'=>$a]; else return ['response_status'=>'error', 'response_msg'=> 'No hay datos'];
            break;
         case 1:
            if(count($a) != 0 ) return ['status'=>'ok', 'msg'=>$a]; else return ['status'=>'error', 'msg'=> 'No hay datos'];
            break;
         case 2:
            if(isset($a->num_rows) && $a->num_rows != 0) return ['status'=>'ok', 'msg'=>$a]; else return ['status'=>'error', 'msg'=> 'No hay datos'];
         break;
      }
   }
