...
//$this->_view->setJs(array('jquery-1.9.0', 'bootstrap.min', 'popper.min', 'fontawasome-ico', 'cUsuariosJquery', 'tablesorter-master/jquery.tablesorter'));
    ...
    public function validaContraseña($a)
    {
        ...
        if (!isset($_POST['token'])  || $_POST['token'] == '') {
            $error = 'Token nesesario, solicite su token con soporte sicloud';
        }
        if (isset($_POST['token']) && ($token != $pToken )) {
    ...
    public function misdatos()
    {
        ...
        if (isset($_POST) && count($_POST) != 0) {
            if (!isset($_POST['accion'])) {
                ...
                if ($b) {
                            $_SESSION['message'] = 'Acutalizo sus datos';
                                $_SESSION['color'] = 'success';
                } else {
                ...
                //   Controller::ver('dd',1,1,'ENTRO');
                    ...
                    if ($b) {
                                $_SESSION['message'] = 'Registro dirección ' . $this->getSql('direccion') ;
                            $_SESSION['color'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Error al registrar dirección';
                        $_SESSION['color'] = 'danger';
                    }
