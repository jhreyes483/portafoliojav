...
class adminController extends Controller
{
        ...
        if (isset($_POST['accion'])) {
            switch ($_POST['accion']) {
                case 'deleteNotific':
                    ...
                    if ($r) {
                        $_SESSION['message'] = 'Elimino log';
                        $_SESSION['color']  = 'success';
                    } else {
                        $_SESSION['message'] = 'Error al eliminar log';
                        $_SESSION['color'] = 'danger';
                    }
                    break;
                case 'notificLeida':
                    ...
                    if ($r) {
                        $_SESSION['message'] = 'Update exitoso exitosa';
                        $_SESSION['color']  = 'success';
                    } else {
                        $_SESSION['message'] = 'Error, aupdate';
                        $_SESSION['color']  = 'danger';
                    }
                    break;
            }
    ...
    public function edit()
    {
        if (isset($_POST['pass'])) {
            ...
            if (!isset($token)) {
                $error = 'Token necesario, solicite su token con soporte Sicloud ';
            }
            if ($token != $pToken) {
                $error =  'Token incorrecto, solicite su token con soporte sicloud';
            }
            if (!isset($error) && !isset($pass)) {
                $error = ' El campo contraseña es requerido';
            }
            if (!isset($error) &&  isset($id)  && empty($id)) {
                $error = 'No seleciono usuario para el cambio de contraseña';
            }
            if (!isset($error)) {
