...
public function  verPagos($param){
  ...
  $sqlAnex = implode(' ' ,$param);
   $sql = 'SELECT  P.fecha , "" as "nada", 
   U.nom1, U.nom2, U.ape1, U.ape2,
   P.actor , P.estado as estadoPago, P.valor,
   TE.nom_egreso,
   PP.nom_pago_prog, PP.estado as estadoPagoProg,
   P.id_pago , MP.nombre_m, P.naturaleza
   FROM pago P
  ...
  '.$sqlAnex;
     Controller::verLogApi('VerPagos', $sql);
...
public function saldoBanco($idUser){
   $sql = 
   'SELECT C.id_cuenta, C.num_cuenta,C.saldo , C.estado, C.fk_user, C.nom_cuenta, C.num_cuenta
   FROM cuenta C
   INNER JOIN usuario U ON C.fk_user = U.ID_us 
   WHERE C.fk_user = '.$idUser;
   $c = $this->db->prepare($sql);
   $c->execute();
   return $c->fetchAll();
}

public function insertMov($a){
   //Controller::Ver($a,1);
   $sql = 'INSERT INTO pago (id_pago, fecha, actor, estado, valor, fk_user, fk_egreso, fk_motivo, naturaleza) 
   VALUES (NULL, 
    "'.date('Y-m-d H:i:s').'", "'.$a[0].'", '.$a[1].', '.$a[2].', "'.$a[3].'", '.$a[4].', '.$a[5].', "'.$a[6].'")';
    // INSERT INTO pago (id_pago, fecha, actor, estado, valor, fk_user, fk_egreso, fk_motivo, naturaleza)VALUES (NULL,"2021-11-14 19:58:26", "Gastos varios", 2, 40000, "1030607384", 1, 7, "D")
   $c = $this->db->prepare($sql);
   return $c->execute();
...
//   foreach($datosModel as $i =>  $d ){
   ...
   public function loginCorreo($d){
      //Controller::ver($d[0], 1,1,'modelo');
      $sql =
      "SELECT U.* , TD.ID_acronimo , 
      RU.estado , 
      R.ID_rol_n , R.nom_rol 
      FROM tipo_doc TD 
      JOIN usuario         U   ON TD.ID_acronimo = U.FK_tipo_doc 
 ...
 */
   ...
   public function  selectUsuarioFac($id=1,  $tipo = 0){
      ...
      $c->execute();
      switch ($tipo) {
         case 0: return $c->fetchAll();                 break;
         case 1: return $c->fetchAll(PDO::FETCH_ASSOC); break; // Api
       }
  }
   ...
   public function conteoUsuariosInactivos(){
     ...
     foreach($r as  $d){
      ...
      return $con;
        ...
        //busqueda por ID
  public function selectIdUsuario($us, $tipo = 0){
