...
// consulta eventos
    ...
    public function m_insert()
    {
        if (!isset($_POST['equipo'])  || $_POST['equipo'] == '') {
            $this->error  = 'No selecciono equipo';
        }
        if (!isset($_POST['tipo'])    || $_POST['tipo']   == '') {
            $this->error  = 'No selecciono tipo de reunion';
        }
        if (!isset($_POST['txtFecha']) || $_POST['txtFecha'] == '') {
            $this->error  = 'No selecciono fecha';
        }
        if (!isset($_POST['txtHora']) || $_POST['txtHora'] == '') {
            $this->error  = 'No digito hora';
        }
        if (!isset($_POST['txtTitulo']) || $_POST['txtTitulo'] == '') {
            $this->error  = 'No digito titulo';
        }
        @session_start();
        $p[0] = ' reuniones ';
        $p[1] = [
