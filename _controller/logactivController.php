...
class logactivController extends Controller{ 
   ...
   public function dataAvance(){
      ...
      if($r->num_rows != 0 ){
         ...
         foreach ($bloques as $i => $d) {
            ...
            $item     = $d[0][2]; 
            // user $d[0][3]
            if($d[0][1] == 'I') $ini =  $d[0][0];
            if($d[1][1] == 'T') $fin =  $d[1][0];
   ...
   public function operaTiempos(array $a){
      ...
      //--------------------------------------------
      // agrupa 
      foreach ($r as $d) $g[$d[3]][$d[40]][$d[30]][] = $d; // agrupa por usuario y por tipo de actividad
   ...
   public function human_time($input_seconds, $rs=1, $normal=0, $mostrarD=0) { //$rs = muestra segundos // normal=1 deja h m s  como abreviaturas // mostrarD=1 muestra d�as
