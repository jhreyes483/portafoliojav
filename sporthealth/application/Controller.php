...
abstract class Controller
{
    ...
    public function __construct($public = null)
    {
        ...
        $this->statusCita = ['S' => 'Solicitud de asignacion','A' => 'Cita asignada','R' => 'Cita rechazada'];
        if (!isset($public)) {
            $this->issetSession();
        }
...
//echo 'ruta modelo  -> ' . ROOT;
  ...
  //limpia los string de codigo sql sanitizar la contrasena por post
    ...
    protected function redireccionar($ruta = false)
    {
        if ($ruta) {
            ...
            //echo "<script>window.location.href='" . BASE_URL . $ruta . "'</script>";
            exit;
        } else {
    ...
    {
      //Controller::ver($this->_request->_parametros);
      //if( (isset($this->_request->_parametros)) && is_array($this->_request->_parametros)   &&  count($this->_request->_parametros) != 0){
  ...
  //filtro para enteros enviados por url
    ...
    protected function verificarAcceso()
    {
        ...
        switch ($_SESSION['usuario']['fk_rol']) {
            case 'D':
                                                                                                         $this->redireccionar('nutricion');
                $_SESSION['color']   = 'success';

                break;
            case 'C':
                                                                                                     echo '<script>alert("perfil no desarrollado")<script>';
   ...
   *******************************************************************************/
    ...
    protected function m_trae_array($a, $tipo = 0)
    {
        if (isset($a) && !empty($a)) {
            $i = 0;
