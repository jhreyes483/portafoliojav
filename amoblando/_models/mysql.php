...
if (!isset($_SESSION['s_vEmp'])) {
    @session_start();
}
if ($_SERVER['SCRIPT_NAME'] == '/incl/mySQLi.class.php') {
    die();
}
...
if (!isset($_SESSION['s_vEmp']) || $_SESSION['s_vEmp'] == '' || !isset($_SESSION['s_sysIneditto'])) {
    die('<script> alert("Su sesi�n ha caducado, por favor ingrese de nuevo.")
		self.location="/";
	</script>');
}
    ...
    function __construct($datos = '')
    {
        ...
        if (!isset($this->conexion)) {
           //if($datos=='') $datos = $_SESSION['s_sysIneditto'].'/datos_conex.php';
            if ($datos == '') {
                $datos = ($_SERVER['DOCUMENT_ROOT'] . '/incl/datos_conex5.php');
            }
