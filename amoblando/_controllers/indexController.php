...
// Guarda notificaicones en varible $this->_view->notificacion 
   ...
   public function carrito(){
      ...
      foreach($tmp  as $c )                $this->_view->credits[$c[0]] = $c[3];
      $tmp                               = $this->db->verPagos();
      foreach($tmp  as $c )                $this->_view->payment[$c[0]] = $c[1];
   ...
   public function registrar(){
      ...
      foreach( $tmp as $d ) $this->_view->doc[$d[0]] = $d[1];
      $tmp = $this->db->verRol();
      foreach( $tmp as $d ) $this->_view->tem[$d[0]] = $d[1];
   ...
   public function catalogo(){
      if(isset($_REQUEST['c'])){
         $subC = [ 6=>1, 7=>1, 8=>1, 9=>3, 10=>2, 11=>2, 12=>2, 13=>2];     
          ...
          foreach( $c as $d )  $this->_view->categorias[$d[0]]=$d[1];
